...
通过证明，我们确认命题逻辑对于等价关系具有**同余性质**（Well-defined）。利用引理，我们可以直接推导出替换规则：

**推论：等价替换推导**
$$ \frac{\Gamma \vdash S \quad \Gamma \vdash P \leftrightarrow Q}{\Gamma \vdash S[P \backslash Q]} 
$$ 

证明路径非常直观：由定理知 $\Gamma, P \leftrightarrow Q \vdash S \leftrightarrow S'$，再利用引理 1 即可从 $S$ 推出 $S'$。这样我们就将逻辑从“论证的艺术”提升为了“演算的科学”。

---

## 5. 为什么这很重要？

等价替换定理宣告了：**在推理的过程中，逻辑关注的是命题的“功能”而非“长相”。**

有了它，我们可以像做代数简便运算一样：识别局部、寻找等价物、实施替换。它是所有逻辑简化规则能够被实际应用到复杂式子中的合法性前提。

在从 1.3 到 1.7 节中证明的所有定律（如德·摩根定律、排中律等），原本只能用于处理整个命题。而现在，它们变成了 **“全威力版”** 的工具——你可以将这些定律应用到任何复杂命题的**任何局部**。这种“局部手术”的能力，是逻辑系统处理大规模命题公式的保证。
