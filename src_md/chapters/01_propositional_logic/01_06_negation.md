...
我们在做几何题时，一个常用的逻辑是：如果只有两种可能性 $P$ 和 $Q$，而我们排除了 $P$，那么 $Q$ 一定成立。在逻辑学中，这被称为**选言三段论**，也就是我们常说的**排除法**。
...
这被称作**爆炸原理**：**从一对矛盾出发，我们可以推导出世界上的任何命题。** 这在直觉上有时会让初学者不适，也是命题逻辑中最反直觉的部分，但这也的确是排除法的自然推论。这告诉我们如果上下文中有一对矛盾，那么逻辑推理将失去分辨真假的能力，也就没有用了。
...
这意味着，**所有的矛盾在逻辑上都是等价的**：一旦我们推出了其中一个，我们就推出了所有。因此，在逻辑学看来，它们都指向同一个终点，我们用一个统一的符号 **$\perp$** 来代表这个状态。
...
我们发现，这种“蕴涵矛盾式的定义”与我们从几何直觉出发的“从排除法来定义”是完全互通的。我们可以把之前基于排除法的矛盾推理写成：$P, \neg P \vdash \perp$，使用一次蕴涵引入就可以得到 $\neg P \vdash P \to \perp$。这说明这种定义对于实现排除法是**必要的**。

现在我们来证明反方向，即这种定义对于排除法也是**充分的**，也就是只要承认这个定义，我们就能推导出排除法。
### 证明：否定的定义可以实现排除法功能
...
利用刚才定义的 $\neg P \equiv P \to \perp$，我们可以证明一个在几何推理中极度重要的工具：**逆否命题**。
...
在几何中，“如果一条直线不能把另一条线段平分，或者不与这条线段垂直”，那么我们可以断定“它不是这条线段的垂直平分线”。这在逻辑中是一种否定算符与合取/析取的互动，这样的关系一共有四种，被称为**德·摩根定律**。
...
（注：这个定理揭示了蕴涵关系和析取的关系，一个蕴含如果成立，那么要么前提不成立，要么结论成立。请尝试利用习题 1.6.2 的结论 $\neg \neg P$ 和排除法来完成证明。这又是一个命题逻辑中反直觉的例子，在许多逻辑教科书中会花大量文字讨论这种感觉，但其实它与爆炸原理是同源的。）
